#!/usr/bin/env python3
"""
LLM integration for CV Parser Agent.
...
try:
    from .api_clients import AnthropicAPIClient, OpenAIAPIClient, JSONResponseParser
except ImportError:
    from api_clients import AnthropicAPIClient, OpenAIAPIClient, JSONResponseParser


class CVParserLLMClient:
    """LLM client specifically for CV parsing operations."""
    ...
    def _init_api_clients(self):
        ...
        if openai_api_key:
            try:
                ...
                self.openai_client = OpenAIAPIClient(client, self.model_name, self.config)
    ...
    def parse_cv_with_llm(self, cv_text: str, parsing_prompt: str) -> Dict[str, Any]:
        """Parse CV using configured LLM provider."""
        ...
        try:
            if self.llm_provider == 'anthropic' and self.anthropic_client:
                content = self.anthropic_client.call_api(cv_text, parsing_prompt)
                return JSONResponseParser.parse_json_response(content)
            elif self.llm_provider == 'openai' and self.openai_client:
                content = self.openai_client.call_api(cv_text, parsing_prompt)
                return JSONResponseParser.parse_json_response(content)
        except Exception as e:
        ...
        if GracefulDegradation:
            ...
            return GracefulDegradation.fallback_cv_parsing(cv_text)
        else:
